// src/pages/specialties/[specialty]/symptoms/index.jsx
...
async function loadMeta(specialty) {
    try {
        const module = await import(`@/data/specialties/${specialty}/symptoms/_meta.js`);
...
export default function SymptomsIndexPage({ specialty, items }) {
    const router = useRouter();
    if (router.isFallback) return <div>Loading...</div>;
    if (!items?.length) return <div className="text-center py-10 text-gray-600">No symptoms found.</div>;
    ...
    return (
        <>
            <SEO
                title={`Symptoms | ${specialty.charAt(0).toUpperCase() + specialty.slice(1)} | Docsy`}
                description={`List of symptoms in ${specialty}.`}
                url={`https://mydocsy.com/specialties/${specialty}/symptoms`}
            />
                ...
                <section className="bg-white py-12 px-6 text-center">
                    <div className="max-w-4xl mx-auto">
                        <h2 className="text-3xl font-bold text-gray-900 mb-6">
                            <span className="text-teal-600">Symptoms</span>
                        </h2>
                        <p className="text-lg text-gray-600">Browse symptoms in {specialty}.</p>
                    </div>
                </section>
                ...
                <div className="grid gap-6 grid-cols-[repeat(auto-fit,minmax(280px,1fr))]">
                    {items.map(item => (
                        ...
                        >
                            ...
                            <a
                                href={`/specialties/${specialty}/symptoms/${item.slug}`}
...
// Dynamically generate paths for all specialties
